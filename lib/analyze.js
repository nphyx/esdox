/**
 * @module analyze
 * Helper functions for analyzing JSDoc ASTs.
 *
 * Handles sorting, adding extra metadata, string escaping / cleanup, and other
 * transforms of raw AST data for use by the mustache templates. These are all
 * used internally and marked as private, but exported for unit testing.
 * purposes.
 */

/**
 * Transforms the AST into a form that represents a single file with modules and their functions.
 *
 * @private
 * @param {Object} ast
 * @param {Object} opts Command-line args
 * @returns {Object}
 *
 * @example
 * { functions:
 *    [ { name: 'testNamed',
 *        params: [ { name: 'file', type: 'String', value: 'filename to parse' } ],
 *        returns: '',
 *        version: '',
 *        description: 'This is a test function\nwith a description on multiple lines' },
 *      { name: 'testAnonynous',
 *        params: [],
 *        returns: 'the result',
 *        version: '',
 *        description: 'function without name',
 *        type: 'String' } ],
 *   methods: [],
 *   classes: [],
 *   modules:
 *    [ { name: 'test_module',
 *        functions:
 *         [ { name: 'testAnonynous',
 *             params: [],
 *             returns: 'the result',
 *             version: '',
 *             description: 'function without name',
 *             type: 'String' } ],
 *        classes: [],
 *        description: '' } ],
 *   global_functions:
 *    [ { name: 'testNamed',
 *        params: [ { name: 'file', type: 'String', value: 'filename to parse' } ],
 *        returns: '',
 *        version: '',
 *        description: 'This is a test function\nwith a description on multiple lines' } ],
 *   description: 'Some extra text\nSome more extra text',
 *   overview: 'This is the overview',
 *   copyright: '2012 Blah Blah Blah',
 *   license: 'MIT',
 *   author: 'Joe Schmo',
 *   version: ''
 * }
 */
module.exports = function analyze(ast, opts) {
  var result = {
    functions: [],
    methods: [],
    classes: [],
    modules: [],
    members: [],
    globalModule: null,
    description: '',
    overview: '',
    copyright: '',
    license: '',
    author: '',
    version: '',
    hasMembers: false
  };
  var currentModule   = null;
  var currentClass    = null;
  var currentFunction = null;
  var ignoreInternal = !opts.All;

  /**
   * Sets up the "Global" catch-all module.
   *
   * @internal
   * @private
   */
  function initGlobalModule() {
    var global = {};
    global.name      = 'Global';
    global.functions = [];
    global.classes   = [];

    result.modules.push(global);
    result.globalModule = global;
  }

  if (!ast) {
    return null;
  }

  ast.forEach(function (tag) {
    if (ignoreInternal && isInternal(tag)) { return; }
    switch (tag.kind) {
      case 'file':
        result.license   = tag.license;
        result.author    = tag.author;
        result.copyright = tag.copyright;
        result.overview  = noLineBreaks(tag.description);

        (currentFunction || result).version = tag.version;
        (currentFunction || result).deprecated = tag.deprecated || true;
        break;
      case 'function':
        if (tag.undocumented) { break; }

        var fn = tag;
        fn.params       = tag.params || [];
        fn.params.forEach(function (p) {
          p.nested = (p.name.indexOf('.') > -1);
          p.description = noLineBreaks(p.description);
        });
        fn.hasParams    = !!fn.params.length;
        // For the function signature
        fn.paramsString = fn.params.filter(function (p) {
          // Filter out nested params
          return !p.nested;
        }).map(function(p) {
          return p.name;
        }).join(', ');

        // For param details
        fn.params.forEach(setPipedTypesString);
        // metadata to support a defaults column in params only if at least one param has a default
        fn.hasDefaultParams = fn.params.reduce((acc, param) => acc || param.defaultvalue !== undefined, false);
        fn.returns      = tag.returns || [];
        fn.returns.forEach(function(ret) {
          setPipedTypesString(ret);
          ret.description = ret.description ? noLineBreaks(ret.description) : false;
        });
        // To avoid reaching to the parent for these fields
        fn.version      = tag.version || false;
        if (tag.fires) {
          fn.fires      = (tag.fires || [])
            .map(fires => ({ref: fires, name: fires.replace("module:", "").replace("event:", "")}));
        }
        fn.description  = noLineBreaks(tag.description);
        fn.deprecated   = tag.deprecated || false;
        fn.moduleName   = currentModule ? currentModule.name : '';
        fn.examples     = (tag.examples || []).map(wrapExample);
        currentFunction = fn;
        if (currentClass) {
          currentClass.methods.push(fn);
          fn.className = currentClass ? currentClass.name : '';
        } else if (currentModule) {
          currentModule.functions.push(fn);
        } else {
          if (!result.globalModule) {
            initGlobalModule();
          }
          result.globalModule.functions.push(fn);
        }
        result.functions.push(fn);
        break;
      case 'member':
        if (currentClass && tag.undocumented !== true) {
          currentClass.members.push(tag);
          setPipedTypesString(tag);
        } else if (tag.scope === 'inner' && tag.undocumented !== true) {
          result.members.push({member: tag.name});
          result.hasMembers = true;
        }
        break;
      case 'return':
      case 'returns':
        if (currentFunction) {
          currentFunction.returns = tag.text;
          currentFunction.type = tag.type;
        }
        break;
      case 'module':
      case 'namespace':
        var module = {};
        module.name = tag.name;
        module.longname = tag.longname;
        module.examples = tag.examples || [];
        module.functions = [];
        module.classes = [];
        module.description = noLineBreaks(tag.description);
        module.requires = tag.requires || [];
        module.hasRequires = !!module.requires.length;
        module.requires.forEach(function(r, i) {
          if (!r) { return ''; }
          module.requires[i] = {req: r};
        });
        result.modules.push(module);
        currentModule = module;
        currentClass = null;
        break;
      case 'class':
      case 'interface':
        var klass = {};
        klass.name = tag.name;
        klass.longname = tag.longname;
        klass.methods = [];
        klass.description = noLineBreaks(tag.description);
        klass.members = tag.properties || [];
        klass.members.forEach(setPipedTypesString);
        klass.params = tag.params || [];
        result.classes.push(klass);
        if (currentModule) {
          currentModule.classes.push(klass);
        } else {
          if (!result.globalModule) {
            initGlobalModule();
          }
          result.globalModule.classes.push(klass);
        }
        currentClass = klass;
        break;
    }
  });

  result.classes.forEach(function(klass) {
    klass.hasMembers = !!klass.members.length;
  });

  // add a hasMultipleModules metadata tag to results if needed
  result.hasMultipleModules = result.modules.length > 1 ? true : false;

  return result;
};

/**
 * Attaches a 'typesString' pipe-separated attribute
 * containing the node's types
 * @private
 * @param {Object} node - AST node with a `types` attribute to process
 * @return {String}
 */
function setPipedTypesString(node) {
  if (!node.type) { return ''; }

  node.typesString = node.type.names.join(" \u2758 "); // safe unicode pipe character
}

/**
 * Checks whether an AST node is private.
 * @private
 * @param {Object} node the AST node to examine
 * @return {Boolean}
 */
function isInternal(node) {
  return node && ((node.name && node.name.lastIndexOf('_', 0) === 0) || node.access === 'private');
}

/**
 * Replaces line breaks with spaces in a string. Used in places where line breaks
 * would mess up Markdown output (like descriptions).
 * @private
 * @param {String} str the string to transform
 * @return {String} the processed string
 */
function noLineBreaks(str) {
  return str && str.replace("\n", " ").replace("\r", " ");
}

/**
 * Intelligently wraps @examples in Github codeblock backticks.
 *
 * Ignores example blocks that already have a user-supplied backtick. It's
 * better to provide named code so the syntax highlighting looks nicer.
 * @private
 * @param {String} str
 * @returns {String} str, wrapped as a code block if necessary
 */
function wrapExample(str) {
  if (!str.match(/^```.*```$/)) {
    str = "```\n" + str + "\n```";
  }
  return str;
}
